...
for font in fonts:
    if 'danger' in font.lower():
        print(font)

# Fonts
impact_font = pygame.font.SysFont(name='katerlin', size=130, italic=True)
danger_font = pygame.font.SysFont(name='dangernightpersonaluse', size=80)

# Render text
impact_font = impact_font.render("|Salentine|", True, 'black')
danger_font = danger_font.render("Play to Death!", True, 'blue')

# Text Entity
impact_font_entity = impact_font.get_rect()
danger_font_entity = danger_font.get_rect()
impact_font_entity.center = (400, 100)
danger_font_entity.center = (400, 200)
...
while running:
    ...
    # Render
    screen.blit(impact_font, impact_font_entity)
    screen.blit(danger_font, danger_font_entity)
